	...
	def cached_images(self):
		...
		try:
			for item in self.sites_config:
				if self.sites_config[item]["image"]["option"]=="custom":
					url_img=self.sites_config[item]["image"]["img_path"]
					print(url_img)
					img_name=url_img.split("/.")[1]
					req=urllib2.Request(url_img)
			...
			self._debug("Cached images",str(e))
			pass			
