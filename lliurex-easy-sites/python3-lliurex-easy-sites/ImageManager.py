...
def scale_image(image_file,x,y,aspect_ratio=True):
	...
	if aspect_ratio:
		...
		pixbuf=pixbuf.scale_simple(x,y,GdkPixbuf.InterpType.HYPER)
...
def create_banner(icon_file,size_x,size_y,txt,frame=True,output_file=None):
	...
	if offset_y==0 and (txt!=None and len(txt)>0):
		offset_y=20
	...
	if txt!=None and len(txt) >1:
		...
		print((size_y-offset_y,size_x,offset_y,offset_x))
	...
	print((px.get_width(),px.get_height(),"????"))
...
if __name__=="__main__":
	...
	create_banner("/usr/share/icons/Antu/apps/48/mozilla-firefox.svg",235,110,"Firefox")